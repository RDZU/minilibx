...
/*   Updated: 2024/06/17 19:04:03 by razamora         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// {
// 	int gd, gm;
// 	double x = 0.01, y ,z, dx,dy,dz, dt= 0.002, t,a = 10, b= 28, c= 8/3;
...
void ft_lorenz_attractor(int x, int y, t_windows *fract) {
    t_complex z;
    double dx, dy, dz;
    double a = 10, b = 28, c = 8 / 3;
    double dt = 0.002; // Time step

    // Calculate initial coordinates based on pixel position and scaling
    z.real = ft_scale_down(x, -2, +2, 0, 799) * (1 / fract->zoom) + fract->move_x;
    z.imaginary = ft_scale_down(y, +2, -2, 0, 799) * (1 / fract->zoom) + fract->move_y;

    for (int i = 0; i < fract->iterations; ++i) {
        // Calculate Lorenz attractor equations
        dx = a * (z.real - z.imaginary) * dt;
        dy = (z.real * (b - z.imaginary) - y) * dt;
        dz = (z.real * z.imaginary - c * z.imaginary) * dt;

        // Update coordinates
        z.real += dx;
        z.imaginary += dy;

        // Skip initial transient (optional for smoother visualization)
        if (i < 10) {
            continue;
        }

        // Calculate color based on iteration count (replace with your desired color scheme)
        int color = ft_scale_down(i, 0, fract->iterations, 0x000000, 0xFFFFFF);

        // Set pixel color in the image buffer (adapt for your image data structure)
...
void ft_mandelbrot(int x, int y, t_windows *fract)
{
	...
	c.imaginary = (ft_scale_down(y, +2, -2, 0, WINDOW_HEIGHT) * (1 / fract->zoom)) + fract->move_y;
		while (i < fract->iterations)
		{
			...
			if ((z.real * z.real) + (z.imaginary * z.imaginary) > 4)
			{
				color =  ft_scale_down (i, YELLOW, ORANGE,0, fract->iterations);
				ft_color(x,y,&fract->img, color);
				return ;
			}
		 	i++;
...
void render(struct s_windows *fract)
{
	...
	while (y++ < WINDOW_HEIGHT - 1)
	{
		...
		while (x++ < WINDOW_WIDTH - 1)
		{
		   ...
		   else if (ft_strcmp(fract->type_fract, "Caos") == 0)
				ft_lorenz_attractor(x,y,fract);
...
// 
//c1r1s4% gcc test_fract.c -lm -Lminilibx-linux -lmlx_Linux -lX11 -lXext